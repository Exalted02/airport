...
function pms_mc_is_paid_plan( $plan_id ) {
    $paid_plans = [2342, 2340, 1350]; // yearly, quarterly, monthly
    return in_array( intval($plan_id), $paid_plans, true );
}

function pms_mc_is_free_plan( $plan_id ) {
    return intval($plan_id) === 2345; // darmowy
}
...
add_action('pms_member_subscription_insert', function() {
	...
	// User is active again → remove deleted_user
    ...
    // GLOBAL premium tag for paid plans
    if ( pms_mc_is_paid_plan($plan_id) ) {
        pms_mc_set_tag($user->user_email, 'premium', 'active');
        // remove downgrade if present
